...
%$$\HH[\Y_G(x)_{o_k}\,|\,\P,\,\ldots] \gg \II[\P;\,\Y_G(x)_{o_k}\,|\,\ldots]$$
$$\II[\P;\,\Y_G(x)_{o_k=(i,j)}\,|\,\{\Y_G(x)_{o_{\ell}}\;|\;\ell<k\},\;
\Y_G(x)_{i\1j}=1\;\text{for all $\;i\1<i$}]=K_G,$$
i.e., the additional information regarding a sample at $x+g_{ij}$
that is given by revealing the partition,
is constant as $i$ and $j$ vary.  
\fi
...
\includegraphics[width=1.3in]{2D/marginal_05}&
\includegraphics[width=1.3in]{2D/energy_05}\\
...
\end{tabular}
\caption{\emph{Exploration of a 2D Scene} 
An explorer (red dot) attempts to efficiently map an unknown scene (a) 
by greedily choosing informative viewpoints (c).  
Observe that we are not merely uncovering area. Once the disposition of a site
has been determined with high confidence (b), that site loses its informative value.}
\end{figure}
...
These bounds depend on some assumed properties of the Ising-model marginal visibility probabilities $p_t(x) = \PP_t[x\in\V]$.  
...
Here we will assume that, for any threshold $\epsilon>0$, 
there is a finite constant $K = K(\epsilon)$, independent of $\V_t$ and $\O_t$, such that
%(some suitable discrete analogue of) 
the boundary curvature of the level set $\{x:p_t(x) \leq 1-\epsilon\}$ is bounded above by $K$.
